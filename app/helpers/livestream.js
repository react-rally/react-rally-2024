...
const isNowBetween = (one, two) =>
  moment.utc().isBetween(moment.utc(one), moment.utc(two));
...
function getLiveStreamURL() {
  ...
  if (moment.utc().isBefore(moment.utc(constants.Dates.CONF_DAY_ONE))) {
    ...
    isNowBetween(constants.Dates.CONF_DAY_ONE, constants.Dates.CONF_DAY_TWO)
  ) {
    url = constants.Links.LIVE_STREAM_DAY_ONE;
  } else if (
    isNowBetween(
      constants.Dates.CONF_DAY_TWO,
      moment.utc(constants.Dates.CONF_DAY_TWO).add(1, "d"),
    )
  ) {
